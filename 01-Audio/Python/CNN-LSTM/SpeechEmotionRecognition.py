...
class speechEmotionRecognition:
    ...
    def mel_spectrogram(self, y, sr=16000, n_fft=512, win_length=256, hop_length=128, window='hamming', n_mels=128,
        ...
        if not np.any(np.isfinite(y)) or len(y) == 0:
            raise ValueError("Invalid audio input: array contains only NaN or Inf values.")
    ...
    ''' Build Time Distributed CNN model '''
    ...
    ''' Predict speech emotion over time from audio file '''
    ...
    def predict_emotion_from_file(self, filename, chunk_step=16000, chunk_size=49100, predict_proba=False,
        ...
        # Z-normalization & clean
        y = np.asarray([np.nan_to_num(zscore(chunk), nan=0.0, posinf=0.0, neginf=0.0) for chunk in chunks])

        # Skip empty chunks
        y = np.array([chunk for chunk in y if np.any(np.isfinite(chunk))])
        if y.shape[0] == 0:
            raise ValueError("All audio chunks are invalid (NaN/Inf).")
